...
public class Asteroid extends Participant implements ShipDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            if(this.getSize() == 2) {
                controller.addParticipant(new Asteroid(Constants.RANDOM.nextInt(3), 1, this.getX(), this.getY(), RANDOM.nextInt(Constants.MAXIMUM_SMALL_ASTEROID_SPEED + 1 - Constants.MAXIMUM_LARGE_ASTEROID_SPEED) + Constants.MAXIMUM_LARGE_ASTEROID_SPEED, controller));
                controller.addParticipant(new Asteroid(Constants.RANDOM.nextInt(3), 1, this.getX(), this.getY(), RANDOM.nextInt(Constants.MAXIMUM_SMALL_ASTEROID_SPEED + 1 - Constants.MAXIMUM_LARGE_ASTEROID_SPEED) + Constants.MAXIMUM_LARGE_ASTEROID_SPEED, controller));
                 }
            if(this.getSize() == 1) {
                controller.addParticipant(new Asteroid(Constants.RANDOM.nextInt(3), 0, this.getX(), this.getY(), RANDOM.nextInt(Constants.MAXIMUM_SMALL_ASTEROID_SPEED + 1 - Constants.MAXIMUM_LARGE_ASTEROID_SPEED) + Constants.MAXIMUM_LARGE_ASTEROID_SPEED, controller));
                controller.addParticipant(new Asteroid(Constants.RANDOM.nextInt(3), 0, this.getX(), this.getY(), RANDOM.nextInt(Constants.MAXIMUM_SMALL_ASTEROID_SPEED + 1 - Constants.MAXIMUM_LARGE_ASTEROID_SPEED) + Constants.MAXIMUM_LARGE_ASTEROID_SPEED, controller));
                 }
            // Expire the asteroid
